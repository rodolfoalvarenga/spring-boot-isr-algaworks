...
public class VeiculoAssembler {
    ...
    public Veiculo toEntity(VeiculoInput veiculoInput) { // dado um VeiculoInput, retorna um Veiculo
        return modelMapper.map(veiculoInput, Veiculo.class);
    }

    public VeiculoModel toModel(Veiculo veiculo) { // dado um Veículo como parâmetro, devemos retornar um VeiculoModel
        return modelMapper.map(veiculo, VeiculoModel.class);
    }

    public List<VeiculoModel> toCollectionModel(List<Veiculo> veiculos) {
        return veiculos.stream()
                .map(this::toModel) // chama o método toModel
                .toList();
    }
